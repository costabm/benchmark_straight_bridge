...
def clone_windspeeds_when_g_nodes_are_diff_from_wind_nodes(windspeed, n_ws_nodes=50, desired_n_g_nodes=51):
    """
    Since AMC delivered a windsim file with half the nodes (50) as the FEM girder nodes (51), this function
    aims to find the nodes which the windsim refer to, and then make nearest neighbor to the remaining nodes
    """
    assert n_ws_nodes == windspeed.shape[1], "This function only works specifically when n_ws_nodes=50"
    n_t_steps = windspeed.shape[2]
    new_windspeed = np.concatenate([windspeed[:,0,:][:,None,:], windspeed], axis=1)
